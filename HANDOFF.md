...
- **§8.3-UI** — la prima sezione frontend dedicata al tutor (Chiara). Il backend è completo: read (students/overview/notebook), write (activities CRUD + note private), AI loop (curator → proposals → approve/reject). Manca tutto il lato visivo: tabella studenti, scheda studente, coda proposte, editor note. Decisione preliminare: costruire in `project/` (HTML+React via CDN, coerente con quanto c'è) o partire direttamente da Next.js (§8.2)? Il primo è più veloce per il pilota, il secondo è investimento.
