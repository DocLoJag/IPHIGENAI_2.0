...
- [ ] **§8.3-UI sotto-tranche 2 (futura)** — feature non incluse nella sotto-tranche 1 ma supportate dal backend: form create activity manuale, edit activity (PATCH fields), override campi in fase di approve proposta, notebook curator esteso paginato. Da fare quando emergono richieste reali dall'uso.
...
- **§8.4 Activity scheduling** — adesso che Chiara può creare task (via approve proposta) o modificarli, manca il pezzo "programma per domani alle 18 e fai apparire automaticamente". Si fa con BullMQ job one-shot su `scheduled_for` oppure (più semplice) lasciando filtrare la query `GET /students/me/home` per `scheduled_for <= now()`. Vedi §8.4.
- **§8.3-UI sotto-tranche 2** — feature aggiuntive supportate dal backend ma non ancora in UI: form create activity manuale, edit activity, override campi in approve, notebook curator paginato. Da fare quando l'uso reale lo richiede.
- **§8.5 SSE streaming chat AI** — UX della chat migliora, ma è cosmetica rispetto al tutor panel ora che il loop AI è chiuso.
