...
- [x] UI upload file lato studente + lato tutor — fatto §8.6-st2a (2026-04-25). Pagina `/files` per Luca, sezione "Allegati" nella scheda di Chiara. Manca: paperclip nel composer chat AI (§8.6-st2b: richiede schema Mongo `ai_messages` con `attachment_ids`) e integrazione AI come `image`/`document` content block all'Anthropic (§8.6-st3).
...
- [x] **§8.6 sotto-tranche 2a — UI upload (lato studente + lato tutor)** (2026-04-25). Pagina `/files` lato studente (link "I miei file" in topbar) con form upload + lista + bottone elimina sui propri file; sezione "Allegati" nella scheda tutor con upload per studente target. `<img crossOrigin="use-credentials">` per le immagini cross-origin. Backend fix collaterale: `GET /uploads` lato student include anche gli allegati con `studentId = self`. Verificato E2E browser. Decisione UX presa: tab dedicato `/files` separato dalla cassetta degli attrezzi (artifact editoriali vs. file caricati dall'utente). Dettagli in §2.
- [ ] **§8.6 sotto-tranche 2b — paperclip nel composer chat AI**:
  - Composer chat AI: paperclip che apre il file picker (riusa `UploadForm`/`api.uploadFile`), anteprima inline prima di inviare, append dell'`attachment_id` al payload del messaggio.
  - Backend: schema Mongo `ai_messages` esteso con `attachment_ids: string[]`. Render dell'allegato dentro la bolla del messaggio AI (thumbnail per immagini, link per PDF).
  - L'integrazione vera col modello Anthropic resta in §8.6-st3.
...
- **§8.6 sotto-tranche 2b — paperclip chat AI** (paperclip nel composer di `/chat/ai` con anteprima, schema Mongo `ai_messages` esteso con `attachment_ids`, render dell'allegato dentro la bolla del messaggio). Step intermedio prima della 3.
