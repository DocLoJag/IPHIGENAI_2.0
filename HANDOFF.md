...
- [x] **Integrazione AI allegati — §8.6 sotto-tranche 3** (2026-04-25, commit `a462b10`, PR #7). Chiude il loop "lo studente fotografa un esercizio → l'AI lo legge davvero". Il workaround di st2 (URL appeso in coda al testo come `📎 nome — /api/uploads/:id`) è stato rimosso: l'allegato viene ora passato come `attachment_ids[]` separato e tradotto in content block `image` (immagini) o `document` base64 (PDF) nell'array `messages` Anthropic. Lo studente vede la chip thumbnail nella propria bolla; l'AI risponde guardando l'immagine, non chiedendola.
  - **Schema Mongo**. Esteso `AiMessageDoc` con `attachment_ids?: string[]` (campo opzionale, presente solo sui doc `from='student'`). Nessuna migration: Mongo non valida lo schema, i doc esistenti restano compatibili (`attachment_ids` undefined è omesso dalla replay come array vuoto). Indici invariati.
  - **Backend — nuovo modulo `services/attachment-blocks.ts`**. Tre helper:
    - `loadAttachmentsForStudent(ids, studentId)` — carica le righe `attachments` per gli id richiesti e applica gli stessi guard di `assertCanAccessAttachment` (`ownerId === studentId OR studentId === self`) + filtro non-soft-deleted. Ritorna le righe nello stesso ordine degli id (così i blocchi conservano l'ordine d'invio del client). Throw 404/403 in modo che la rotta API ritorni un errore JSON pulito PRIMA di toccare Mongo.
    - `loadAttachmentsByIds(ids)` — variante "permissiva" senza guard, usata per la replay dell'history (gli id vengono dai doc Mongo, già validati al loro turno). Soft-deleted filtrati silenziosamente: se Luca cancella un file vecchio, la replay omette il blocco invece di fallire.
    - `gridfsToBase64(gridfsId)` + `attachmentToContentBlock(att, b64)` — download del binario in memoria (no streaming, ok per file ~MB del pilota) e build del block secondo il MIME (`image/{png,jpeg,webp,gif}` → `image`; `application/pdf` → `document`).
  - **Backend — `services/tutor-agent.ts`**. Splittato in `prepareTutorTurn` (validazione + history + costruzione array `messages`) e `finalizeTutorTurn` (persistenza AI doc) era già fatto in §8.5. Qui esteso `prepareTutorTurn` con la pipeline allegati: (1) valida gli id del turno corrente con `loadAttachmentsForStudent` PRIMA di insertare il doc studente (no doc orfani in caso di 403); (2) persiste `attachment_ids` sul doc studente; (3) aggrega gli id di TUTTI i messaggi nella history trimmata, una sola query Postgres + un download GridFS per id (cache `base64Cache` locale al turno); (4) ricostruisce `anthropicMessages`: i messaggi `student` con allegati diventano `content: [<image/document blocks>, {type:'text',text}]`, gli altri restano stringhe. Difensivo: se la replay si ritrova con 0 blocchi (tutti gli allegati cancellati e testo vuoto) inserisce un placeholder `(allegato non più disponibile)` per non rompere l'invariante Anthropic "content non-vuoto".
  - **Backend — rotte chat AI** (`routes/ai-threads.ts`). Sia `POST /api/ai/threads/:id/message` (sync) sia `POST /api/ai/threads/:id/message/stream` (SSE) accettano ora `{ text?: string, attachment_ids?: string[] }`. Validazione zod via `superRefine`: almeno uno tra testo non vuoto o allegato presente — il messaggio "completamente vuoto" ritorna 400 `VALIDATION`. Cap a **5 allegati per turno** (i blob image/document si sommano nel payload Anthropic, vogliamo evitare cost-spike). `GET /ai/threads/current` e l'evento SSE `meta` includono ora `attachments[]` serializzati (forma identica a `routes/uploads.ts` — `id, url, filename, mime, size_bytes, owner_id, student_id, created_at, deleted_at`) per i messaggi che ne hanno. `serializeAttachment` estratto in `routes/serializers.ts` per riuso fra `uploads.ts` e `ai-threads.ts` (no più import incrociato).
  - **Frontend — `pages/AIChat.jsx`**. Il composer ora invia `attachment_ids: [attachment.id]` invece di appendere l'URL al testo. Optimistic update include subito la chip thumbnail nella bolla studente prima della conferma server (sostituita poi dal record definitivo dall'evento `meta`). Fallback POST sync passa anche lui `attachment_ids`.
  - **Frontend — `components.jsx`**. `ChatScreen` legge `m.attachments` (array `SerializedAttachment` dal backend) e renderizza una griglia thumbnail/chip 80×80 sopra il testo (margin-bottom solo se c'è anche testo). Nuovo `MessageAttachment`: per immagini un `<img crossOrigin="use-credentials">` 88×88 cliccabile (apre il file in nuova tab); per PDF una chip "PDF + filename" cliccabile. `crossOrigin="use-credentials"` necessario perché il backend serve i binari su un origin diverso e richiede il cookie httpOnly (gotcha §10).
  - **Cast TypeScript per `document` block**. L'SDK Anthropic 0.32 ha `document` solo nel namespace `beta` ma l'API standard `messages.create` lo accetta a runtime su Sonnet 4.5+. Cast esplicito `as unknown as MessageParam[]` al call site, commento che spiega il perché. Quando l'SDK promuove `document` nel type principale, il cast si rimuove.
  - **Verifiche E2E** contro Railway (dall'agente che ha implementato la tranche, prima del merge di PR #7). Le tracce dei test sono ancora visibili nel thread `ai-thread-luca-math`: PNG con un'equazione (test "AI legge l'immagine"), PDF "Compito: risolvi 5+7" (test "AI legge il PDF e prova a risolvere"), PNG caricato dall'admin per testare il 403 cross-role. Per ripartire da uno stato pulito su queste rotte: `POST /api/admin/reset-demo` (vedi §11). Coperti: happy path image + PDF, validation senza testo né allegato → 400 `VALIDATION`, ownership cross-role → 403, soft-deleted → 404, replay multi-turno con history ricostruita, regressione su `GET /ai/threads/current` (campo `attachments[]` presente solo sui messaggi che ne hanno).
  - **Cosa NON c'è in questa sotto-tranche** (per scope):
    - Cache LRU dei base64 cross-turno: oggi rileggiamo da GridFS ogni turno per ogni allegato della history. Per il pilota (~10 turni a sessione, ~3 MB media) è accettabile, è documentato come "next step se i thread crescono".
    - Allegati lato `/threads` (chat con Chiara): la chat tutor umano resta testuale; per abilitarla basterebbe propagare `enableAttach` a `ChiaraChat` ma non c'è richiesta.
    - Modalità "AI prepara una risposta col PDF citato": l'SDK supporta citazioni nel `document` block (i tipi sono in `beta`); per ora le ignoriamo, l'AI risponde con il testo libero che vede.
    - Compressione/resize automatica delle immagini grandi: il MIME whitelist + cap 10 MB (st1) tiene a freno gli abusi; per ottimizzare i token Anthropic in futuro si potrà rimpicciolire le immagini > 1024px sul lato lungo prima di base64-encodarle.
  - **Gotcha rilevate / confermate**:
    - L'invariante Anthropic "content non-vuoto" si rompe se un turno passato si ritrova con 0 blocchi (es. tutti gli allegati soft-deleted dopo l'invio + testo originario vuoto). Mitigato col placeholder `(allegato non più disponibile)`.
    - Ordine dei blocchi importa: l'API tratta i blocchi nell'ordine del payload. Mettiamo immagini/PDF PRIMA del testo perché il modello legga l'oggetto e poi la domanda — pattern consigliato dalla doc Anthropic.
    - `prepareTutorTurn` valida gli allegati PRIMA dell'insert su Mongo: senza questo ordine un client malizioso potrebbe sporcare la history con doc orfani referenzianti file altrui (anche se la replay li droppa, il doc esisterebbe). Pattern salvato in commento.
...
- [x] Integrazione AI allegati — fatto §8.6-st3 (2026-04-25, commit `a462b10`, PR #7). L'allegato viene passato come content block `image`/`document` all'Anthropic; l'AI legge davvero la foto/PDF invece di rispondere "non vedo le immagini". Loop §8.6 chiuso end-to-end (storage + UI + AI).
...
- [x] **§8.6 sotto-tranche 3 — integrazione AI** (2026-04-25, commit `a462b10`, PR #7). Lo studente passa l'allegato come `attachment_ids[]` separato dal testo nel composer chat AI; il backend lo carica da GridFS e lo accoda come content block `image` (immagini) o `document` base64 (PDF) all'array `messages` Anthropic. Lo studente vede la chip thumbnail nella propria bolla; l'AI risponde guardando davvero l'immagine/PDF, non chiedendola. Ownership cross-role enforced via `loadAttachmentsForStudent`. Schema Mongo `AiMessageDoc` esteso con `attachment_ids?: string[]` (compat con doc esistenti). Replay history ricostruisce i blocchi turno per turno (cache base64 locale al turno; rilettura GridFS ogni turno è OK per il pilota). Cap 5 allegati/turno contro cost-spike. Dettagli completi in §2.
...
- **§8.3-UI sotto-tranche 3b residuo** — notebook curator paginato esteso (oggi solo l'ultima nota nella scheda studente), CRUD utenti via UI admin (oggi via curl). Da fare quando l'uso reale lo richiede. (St3a — ripristino scartati — già fatta; st3b parziale — UI admin minimale — già fatta.)
- **Cleanup batch dei blob GridFS soft-deleted** — nessun job che cancella `attachments.deleted_at < now() - 30d` + il blob GridFS corrispondente. Oggi i soft-deleted restano per sempre. Job batch è ~30 righe, non urgente finché lo storage non cresce.
- **Cache LRU base64 dei content block AI** — la replay dell'history rilegge ogni allegato da GridFS ad ogni turno chat. Per il pilota (singolo studente, ~10 turni a sessione) è accettabile, ma diventerà sensibile se i thread crescono o se gli allegati sono PDF di qualche MB. Cache LRU process-wide o Redis-backed.
