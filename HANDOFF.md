...
- [x] Upload UI — fatto §8.6-st2 (2026-04-25). Paperclip nel composer chat AI + sezione Allegati nel pannello tutor con thumbnail e cancella. Manca: integrazione AI come `image`/`document` content block all'Anthropic (§8.6-st3).
...
- [x] **§8.6 sotto-tranche 2 — UI upload** (2026-04-25). Solo frontend additivo (nessuna modifica backend). `api.upload(formData)` helper + paperclip 📎 in `ChatScreen` (opt-in via `enableAttach`, abilitato in `AIChat`) + `AttachmentsBlock` in `TutorStudent.jsx` con thumbnail per immagini, badge PDF, soft-delete. L'allegato finisce nel testo del messaggio AI come `📎 <filename> — <url>` (la st3 sostituirà con `attachment_ids` veri come content block Anthropic). Verificato E2E in browser via preview MCP: upload da tutor per Luca → comparsa in lista; upload da studente nel composer → chip preview con thumbnail → submit → link in bolla studente; cross-role visibility OK; DELETE soft + reset-demo idempotente. Dettagli in §2.
