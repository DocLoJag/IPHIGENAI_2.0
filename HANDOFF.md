...
- [x] **Cache LRU base64 dei content block AI** (2026-04-26 notte, PR #14). Igiene tecnica: la replay dell'history nei thread chat AI rileggeva ogni allegato da Mongo GridFS ad ogni turno (vedi nota di costo in §8.6-st3). Ora c'è una cache process-local in `services/attachment-blocks.ts`.
  - **Backend** (`backend/src/services/attachment-blocks.ts`). `gridfsToBase64(gridfsId)` ora consulta `Map<gridfsId, {data, bytes, lastAccess}>` con cap a 50 entries / 50 MB e eviction LRU per `lastAccess` crescente. Su miss legge da GridFS, mette in cache, evict greedy se sopra una delle due soglie. Niente Redis: 1 sola replica API in produzione + cache miss costa poco.
  - **Invalidazione**: esposto `invalidateAttachmentCache(gridfsIds?: string[])`. Chiamato da `routes/admin.ts /admin/cleanup-attachments` (dopo hard-delete dei blob, evita di servire da cache binari ormai inesistenti) e da `seed/run.ts wipe()` (dopo drop delle collezioni GridFS). Esposto anche `attachmentCacheStats()` per debug.
  - **Decisione** sul cap: 50 MB scelto per non saturare il container Railway (default 512 MB) lasciando margine per Anthropic SDK + Fastify + base64 di un singolo turno. Quando si scalerà a più repliche, switch a Redis-backed è additivo (l'API del modulo resta identica).
  - **Verifica E2E** (browser preview locale `:5173` → API Railway, post-deploy):
    - Typecheck backend verde.
    - PNG 64×64 con scritto "5+7" generato via canvas → upload come Luca → 3 turni successivi sulla chat AI con replay history (turno 1 con `attachment_ids`, turno 2 e 3 senza ma replay rilegge gli allegati dei turni precedenti). t1=2909 ms (cache miss + Anthropic), t2=2178 ms (cache hit replay + Anthropic), t3=1705 ms. Nessun 500 sul path replay → cache + invalidation funzionano end-to-end.
    - Gotcha rilevata in test E2E: Anthropic Messages API rifiuta PNG sintetici troppo piccoli (es. 1×1 trasparente, 67 byte) con `400 invalid_request_error: Could not process image`. Per i test futuri di image content blocks usare PNG ≥ 64×64 con contenuto visibile, possibilmente via canvas. Salvato come pattern.
  - **Reset-demo invalida la cache**: dopo `seedDemo()` il drop delle collezioni GridFS lascerebbe la cache che riferisce a id non più esistenti. Il wipe ora chiama `invalidateAttachmentCache()` per pulire l'intera mappa.
- [x] **Cleanup batch attachments soft-deleted** (2026-04-26 notte, PR #13). Chiude un debito tecnico rimasto da §8.6-st1: oggi i soft-deleted restavano per sempre in GridFS + Postgres. Now si lancia manualmente.
  - **Backend** (`backend/src/routes/admin.ts`). Nuovo endpoint `POST /api/admin/cleanup-attachments` (admin guard). Body `{days?: number}` zod `.strict()` con `min(7).max(3650)`, default 30. Logica: `SELECT * FROM attachments WHERE deleted_at IS NOT NULL AND deleted_at < threshold` → per ogni candidato `attachmentsGridFS().delete(new ObjectId(gridfsId))` (best-effort, log warn su fail) → `DELETE FROM attachments WHERE id IN (...)`. Ritorna `{ok, days, threshold, candidates, blobs_deleted, blobs_failed, rows_deleted}`. Log warn con actor + counts per audit.
  - **Sicurezza**: la replay history dei thread chat AI usa `loadAttachmentsByIds` che filtra silenziosamente gli `attachment_ids` non trovati o soft-deleted (vedi §8.6-st3), quindi hard-delete dopo la grace period non rompe i thread esistenti — semplicemente non rivedi più quel blocco.
  - **UI admin** (`project/app/pages/AdminHome.jsx`). Card "Pulizia allegati" sotto "Reset demo". Stesso pattern del reset: bottone "pulisci allegati" → click 1 mostra "sicuro? clicca ancora" per 4s → click 2 esegue. Toast con counts di ritorno (es. "Pulizia: 3 allegati cancellati (3 blob, 0 falliti)") oppure "Nessun allegato da pulire (soft-deleted da almeno 30 giorni)".
  - **Verifiche E2E** (browser preview locale `:5173` → API Railway):
    - Endpoint `cleanup-attachments` con candidates=0 → `{ok:true, days:30, candidates:0, blobs_deleted:0, rows_deleted:0}`.
    - Body `{days:1}` → 400 `VALIDATION` (sotto il min=7).
    - Body `{foo:"bar"}` → 400 `VALIDATION` (zod strict).
    - Senza cookie → 401 `AUTH_REQUIRED`. Cookie student → 403 `FORBIDDEN` (guard requireRole admin).
    - File appena soft-deletato non viene cancellato neanche con `days:7` (deleted_at è di pochi secondi). Comportamento corretto della grace period.
    - UI: card "Pulizia allegati" presente nell'ordine corretto, doppio-click di conferma funziona, toast con copy "Nessun allegato da pulire" con candidates=0.
  - **Cosa NON c'è in questa sotto-tranche** (per scope, rimandato): cron automatico (BullMQ scheduled job o railway-cron) — il pilota ha pochi file e la chirurgia umana è preferibile. Quando lo storage cresce, basta aggiungere un job BullMQ che invoca lo stesso path interno (refactor in funzione `cleanupAttachments(days)` riusabile). Endpoint per lista preview (dry-run) — il behavior è "trim-only" e l'admin può sempre fare `/api/uploads?include_deleted=true` se in futuro lo aggiungiamo.

- [x] **CRUD utenti via UI admin — §8.3-UI sotto-tranche 3b residuo** (2026-04-26). Chiude lo step finale di §8.3-UI: oggi l'admin gestisce gli utenti dal browser invece che via curl. Solo frontend additivo, zero backend (POST/GET/PUT/DELETE su `/api/admin/users` esistevano già dall'inizio del progetto).
  - **Frontend** (`project/app/pages/AdminHome.jsx`). Refactor di `AdminHomePage` da "lista read-only" a "lista CRUD". Nuova `UserCard` (sostituisce il render inline che prima aveva `UserGroup`) con bottoni `modifica` e `disattiva` per ogni riga. Nuovo componente `UserForm` riusato in 2 modalità: crea (initial null) e modifica (initial = user object).
    - **Crea**: bottone `+ nuovo utente` nell'header lista apre il form vuoto. Campi: select `ruolo` (studente/tutor/admin), `username`, `nome breve`, `nome completo` (opzionale), `iniz. avatar` (opzionale, max 2 char), `password` (min 6). Se ruolo = studente, mostra anche `classe`, `scuola`, `tutor assegnato` (select popolata con i tutor presenti in lista — niente fetch separato, riusa `byRole('tutor')`). Submit → `POST /api/admin/users` (zod backend valida tutto: username regex `[a-z0-9_.-]`, role enum). Errore inline (`err` state) per messaggi user-friendly da `e.message` ZodError o conflict 409 `USERNAME_TAKEN`.
    - **Modifica**: bottone `modifica` su `UserCard` apre il form pre-compilato. Role e username diventano pill non editabili (immutabili dopo creazione lato backend); password vuoto = invariato (placeholder "••••••"). Submit → `PUT /api/admin/users/:id` con solo i campi cambiati (diff vs `initial`). Per studente, `grade`/`school`/`tutor_id` sono editabili.
    - **Disattiva**: bottone `disattiva` (rosso) chiede conferma via `confirm()` con label leggibile. Soft-delete via `DELETE /api/admin/users/:id` (backend setta `disabledAt=now()`, l'utente non può più fare login, sparisce dalla lista). Self-disable bloccato in UI: se `u.id === currentUserId`, bottone disabilitato con tooltip "non puoi disattivare te stesso" e label `(tu)` accanto al nome (l'admin loggato non può disattivarsi nel browser; il backend non ha guard analogo, ma l'utility sandbox è sufficiente per il pilota).
  - **Verifiche E2E** in browser via preview MCP (`:5173` → API Railway):
    - Login admin → `/admin` → bottoni `+ nuovo utente`, `modifica`, `disattiva` visibili. Card admin ha label `(tu)` + bottone disattiva disabilitato con tooltip corretto.
    - Click `+ nuovo utente` → form rendered con 9 input + 2 select (ruolo + tutor); select tutor popolata con `Chiara Rizzo (@chiara)`.
    - **Create flow** (via API): `POST /admin/users` con `role=student`, `username=test_e2e_admin`, `password=temppass123`, `name=Marco`, `full_name=Marco Test`, `avatar_initial=M`, `grade=Terza scientifico`, `school=Liceo Test`, `tutor_id=tutor-chiara` → 201 `{user: { id: 'student-test-e2e-admin', tutor_id: 'tutor-chiara', ... }}`.
    - **Update flow**: `PUT /admin/users/student-test-e2e-admin` con `{name: "Marco2", grade: "Quarta scientifico"}` → 200, lista subsequente mostra `name="Marco2"`, `grade="Quarta scientifico"`.
    - **Delete flow**: `DELETE /admin/users/student-test-e2e-admin` → `{ok: true}`. Lista subsequente non contiene più l'utente (filtro `disabled_at IS NULL` lato backend).
    - **Edit form pre-compilato**: click `modifica` su card di Chiara → form rendered con pill `tutor`, pill `@chiara`, name="Chiara", full_name="Chiara Rizzo", avatar="C", password vuoto (placeholder "••••••"). Annulla → form scompare, lista resta intatta.
    - `/admin/reset-demo` post-test → stato seed ripulito.
  - **Cosa NON c'è in questa sotto-tranche** (per scope, rimandato): UI per riattivare un utente disattivato (`disabledAt = null`). Il backend non ha endpoint dedicato per riattivare, e la lista `GET /admin/users` filtra `disabled_at IS NULL` quindi gli utenti soft-deleted spariscono completamente dalla UI. Per il pilota va bene; se serve riattivazione si aggiunge un endpoint `POST /admin/users/:id/restore` + un toggle "mostra disattivati" nella lista. Roles editing: oggi il role è immutabile post-creazione (non si può promuovere uno studente a tutor); coerente con il backend (`PUT` non accetta `role`). Cambio username: stesso ragionamento. CSRF/permessi granulari sui campi: l'admin oggi è uno solo, non urgente.
  - **Gotcha rilevata**: il backend `POST /admin/users` ritorna 201 con `{user: {...}}` (non `{id, ...}` flat). Inizialmente avevo provato `await onSubmit(payload)` aspettandomi che il chiamante gestisse, ma siccome `create` in `AdminHomePage` chiama `await api.post()` e poi `reload()`, va già bene così. Salvato come pattern: i nostri serializer admin wrappano sempre in `{user}` sia su POST che su PUT.

- [x] **Notebook curator paginato — §8.3-UI sotto-tranche 3b residuo (parziale)** (2026-04-26). Chiude un gap UX rimasto in piedi dalla st1: la scheda tutor mostrava solo *l'ultima* nota del curator (campo `last_curator_note` nel bundle `/overview`), nonostante l'endpoint `GET /api/tutor/students/:id/notebook?limit=N` fosse pronto da §8.3-READ del 2026-04-23. Ora Chiara può scorrere tutto lo storico per uno studente direttamente in scheda. Solo frontend additivo, zero backend.
  - **Frontend** (`project/app/pages/TutorStudent.jsx`). Nuovo componente `NotebookBlock` montato fra `CuratorNoteCard` e `ProposalsBlock` come continuazione naturale "ultima nota → storico completo". Parte chiuso (bottone `mostra`/`nascondi` simmetrico a `DismissedBlock`); il fetch parte solo all'apertura via `useApi(path, { enabled: open })`. Paginazione incrementale: state locale `limit` a `NOTEBOOK_PAGE=10`, click su "carica altre" alza il limit di 10 e `useApi` rifa la GET (path-keyed dependency). Bottone "carica altre" appare solo se `items.length === limit` (= ci sono potenzialmente altre note dietro); sparisce quando il backend ritorna meno note del limit chiesto. Niente offset/cursor: per il pilota con poche note a studente è lineare e sufficiente.
  - **Componente `NotebookEntry`**: ogni nota in stile compatto (border-top dashed, data in alto, body con `white-space: pre-wrap`, pill `topic` + `conf %`, riga `nodi:` per `signals.stumble_points`, riga `prossimo:` per `signals.next_step_hint`). Stessa struttura di `CuratorNoteCard` ma più asciutta, senza il box card-cream — è un elenco, non un highlight.
  - **Verifiche E2E** in browser via preview MCP locale (`:5173` → API Railway):
    - Login Chiara → scheda Luca → sezione "Storico curator" presente nel DOM tra "ultima nota del curator" e "Proposte da rivedere", chiusa di default (bottone "mostra").
    - Stato vuoto: GET `/tutor/students/student-luca/notebook?limit=10` → `{items:[], total:0}` (il seed non ha note: il curator scrive solo a fine sessione). Click "mostra" → renderizza copy "Nessuna nota ancora. Il curator scrive a fine sessione." → bottone passa a "nascondi".
    - Generazione nota reale: logout Chiara → login Luca → POST `/sessions/sess-2026-04-21-math/answer` su `ex-delta-neg-3` (chiude la sessione, `next_exercise_id=null`) → polling `/notebook?limit=10` come Chiara → curator scrive in **8s** (più veloce dei 30s tipici) → `total=1` con body coerente, `signals.confidence=0.75`, `signals.topic="Equazioni di secondo grado — discriminante"`, 3 `stumble_points`, `next_step_hint`.
    - Reload pagina + click "mostra" → 1 entry resa con: kicker "oggi · 01:29" (formatWhen), body completo (~750 char) con `pre-wrap` rispettato, pill `topic · Equazioni di secondo grado — discriminante` + `conf · 75%`, riga `nodi:` con i 3 stumble joinati con ` · `, riga `prossimo:` con next_step_hint.
    - Bottone "carica altre" non appare (1 < 10 = limit, hasMore=false). Comportamento corretto.
    - Toggle nascondi → `entries=0`, bottone "mostra"; mostra → `entries=1`, bottone "nascondi".
    - `/admin/reset-demo` post-test → stato seed ripulito, sessione di Luca torna `paused` per il prossimo run.
  - **Cosa NON c'è in questa sotto-tranche** (per scope, rimandato): paginazione cursor-based (offset/`written_at < lastSeen`); per ora lineare con limit incrementale è abbastanza per il pilota e l'endpoint backend non lo supporta nativamente. CRUD utenti via UI admin (oggi solo lista + reset; per creare/modificare utenti si usa ancora curl): non urgente, l'admin pilota è uno solo. Notebook esposto anche lato studente: fuori scope per principio (le note del curator sono interne al sistema tutor → studente non le vede).
...
- [x] **§8.3-UI sotto-tranche 3b residuo — notebook curator paginato esteso (2026-04-26)** — `NotebookBlock` lazy con paginazione incrementale (limit di 10, "carica altre" alza il limit) montato fra `CuratorNoteCard` e `ProposalsBlock` in `TutorStudent.jsx`. Endpoint backend `GET /tutor/students/:id/notebook?limit=` esisteva già da §8.3-READ. Verificato E2E con sessione chiusa → curator scrive nota → notebook la mostra con body, signals, stumble_points, next_step_hint. Dettagli sopra in §2.

- [x] **§8.3-UI sotto-tranche 3b residuo — CRUD utenti via UI admin (2026-04-26)** — `UserForm` (riusato per crea + modifica) + bottoni `modifica`/`disattiva` per ogni utente nella lista admin. Endpoint backend `POST/PUT/DELETE /api/admin/users` esistevano già dall'inizio del progetto. Self-disable bloccato lato UI con tooltip + label `(tu)`. Verificato E2E (create/update/delete + form pre-compilato in edit). Dettagli sopra in §2.
...
- **Tutta la roadmap di codice in elenco è atterrata** (al 2026-04-26 notte). UI tutor + admin completa, cleanup batch GridFS in mano all'admin, cache LRU base64 in posto. Restano solo:
  ...
  - **§8.2 Porting a Next.js** — infrastrutturale, rimandabile finché il pilota può girare con l'attuale frontend statico. PWA installabile + SSR + ecosistema componenti.
  - **Frontend deployato su un dominio pubblico** — oggi gira solo in locale per dev. Quando si introducono studenti reali, va deployato come terzo servizio Railway o Vercel/Cloudflare Pages.
  - **Hardening** — test suite (routes/tutor/guards.ts e services/curator.ts sono i posti dove un test salverebbe da incidenti silenziosi), ESLint config completa, observability strutturata (pino sui servizi runtime, oggi `console.log` in curator/worker/redis).
