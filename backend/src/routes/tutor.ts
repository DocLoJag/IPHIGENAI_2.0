...
async function assertTutorOwnsProposal(tutorId: string, proposalId: string) {
  const [p] = await db
    .select()
...
// Nota sulla firma: usiamo `z.ZodTypeAny` + `z.infer<S>` (non `z.ZodType<T>`)
// per far sì che i default di zod vengano riflessi nel tipo di ritorno.
// Es. `z.coerce.number().default(20)` dà output `number`, ma `z.ZodType<T>`
// inferisce T dall'input type (opzionale) perdendo il default.
...
function serializeProposal(p: typeof activityProposals.$inferSelect) {
  return {
    id: p.id,
...
export default async function tutorRoutes(app: FastifyInstance) {
  ...
  app.get('/tutor/proposals', guard, async (req) => {
    const tutorId = req.principal!.sub;
    const { status, limit } = parseBody(proposalsListQuery, req.query ?? {});
  ...
  // Una proposta già decisa (approved/rejected) ritorna 409.
  app.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      const body = parseBody(approveProposalBody, req.body ?? {});
  ...
  // POST /tutor/proposals/:id/reject — rifiuta la proposta, opzionalmente con motivazione
  // breve (utile per fare tuning del curator in futuro).
  app.post<{ Params: { id: string } }>(
    ...
    async (req) => {
      ...
      const body = parseBody(rejectProposalBody, req.body ?? {});
